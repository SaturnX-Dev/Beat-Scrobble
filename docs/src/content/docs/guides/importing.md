...
description: How to import your existing listening history from other services into Beat Scrobble.
---

Beat Scrobble currently supports the following sources to import data from:
- Spotify
- Maloja
- LastFM (using https://lastfm.ghan.nl/export/)
- ListenBrainz
- Koito (Legacy) - Beat Scrobble is fully compatible with its predecessor's data structure.
- Maloja
- LastFM (using https://lastfm.ghan.nl/export/)
- ListenBrainz
...
these imports to go faster, you can [disable MusicBrainz](/reference/configuration/#Beat Scrobble_disable_musicbrainz) in the config while running the importer. However, this
...
[disable MusicBrainz rate limiting](/reference/configuration/#Beat Scrobble_musicbrainz_url) in the config if you want imports to be faster.
:::
...
`import` folder in your config directory, and restart Beat Scrobble. The data import will then start automatically.

Beat Scrobble relies on file names to find files to import. If the files aren't being imported automatically, make sure they contain `Streaming_History_Audio` in the file name.
...
put the resuling `.json` file into the `import` folder in your config directory, and restart Beat Scrobble. The data import will then start automatically.

Beat Scrobble relies on file names to find files to import. If the files aren't being imported automatically, make sure they contain `maloja` in the file name.
...
Once you restart Beat Scrobble, it will automatically detect the file as a Last FM import, and begin adding your listen activity immediately.
...
folder in your config directory. Once you restart Beat Scrobble, your ListenBrainz activity will immediately start being imported.