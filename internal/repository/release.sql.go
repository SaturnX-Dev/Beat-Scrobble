...
func (q *Queries) CountTopReleases(ctx context.Context, arg CountTopReleasesParams) (int64, error) {
	row := q.db.QueryRow(ctx, countTopReleases, arg.ListenedAt, arg.ListenedAt_2, arg.UserID)
...
VALUES ($1, $2, $3, $4)
RETURNING id, musicbrainz_id, image, various_artists, image_source, genres, release_date, popularity, spotify_id, label, release_date_precision, search_vector
`
...
const updateReleaseMetadata = `-- name: UpdateReleaseMetadata :exec
UPDATE releases SET 
  genres = $2,
  release_date = $3,
  popularity = $4,
...
func (q *Queries) UpdateReleaseMetadata(ctx context.Context, arg UpdateReleaseMetadataParams) error {
	_, err := q.db.Exec(ctx, updateReleaseMetadata,
		arg.ID,
		arg.Genres,
		arg.ReleaseDate,
		arg.Popularity,
